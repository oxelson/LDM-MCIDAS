...
void *decode_fsl1(cdfid,miss,iret)
int cdfid;
long miss;
int *iret;
{
...
if(ier == 0)
   ...
   for(i=0;i<unlimsiz;i++)
      ...
      stadat = (prof_struct *)malloc(sizeof(prof_struct));
      if(stadat == NULL)
         {
      ...
      stadat->staName = (char *)malloc(strlen(staName));
      ...
      if((sfc_pres != fill)&&(sfc_pres != norec)) stadat->sfc_pres = sfc_pres;
      if((sfc_temp != fill)&&(sfc_temp != norec)) stadat->sfc_temp = sfc_temp;
      if((sfc_sped != fill)&&(sfc_sped != norec)) stadat->sfc_sped = sfc_sped;
      if((sfc_drct != fill)&&(sfc_drct != norec)) stadat->sfc_drct = sfc_drct;
      if((sfc_dwpc != fill)&&(sfc_dwpc != norec)) stadat->sfc_dwpc = sfc_dwpc;
      if((sfc_rain != fill)&&(sfc_rain != norec)) stadat->sfc_rain_amt = sfc_rain;
      ...
      if(ier == 0)
         ...
         for(j=0;j<NLEV[k];j++)
            ...
            if(plev != NULL)
               {
               plev->level = level;
               if(uwnd == fill) uwnd = fmiss;
               if(vwnd == fill) vwnd = fmiss;
               if(wwnd == fill) wwnd = fmiss;
               if(qual != 10) 
                  {
                  uwnd = fmiss;
                  vwnd = fmiss;
